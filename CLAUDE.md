...
| iOS code-signing fails on `aps-environment` with a free Apple ID | A free Apple ID cannot sign the Push Notifications capability that `expo-notifications` adds | Remove `aps-environment` from `ios/PawStep/PawStep.entitlements` for free-provisioning test builds — see [docs/ios-test-build.md](docs/ios-test-build.md). Regenerated by `expo prebuild --clean`; re-apply after any prebuild. |
| Viro fails to compile on iOS / New Architecture not detected | `ios/.xcode.env` is missing `RCT_NEW_ARCH_ENABLED=1`; ViroReact requires the New Architecture | Add `export RCT_NEW_ARCH_ENABLED=1` to `ios/.xcode.env` (lost on `expo prebuild --clean`, re-apply after) |
